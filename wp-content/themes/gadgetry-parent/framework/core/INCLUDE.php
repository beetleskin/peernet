...
class TF_INCLUDE extends TF_TFUSE {
    ...
    public function css($css_name, $type, $placeholder = 'tf_head', $version = '1.0', $condition_ie = '', $visibility = array()) {
        ...
        if (!in_array($to_store, $this->css))
            if (file_exists($this->types[$type] . '/' . $css_name . '.css'))
                $this->css[] = $to_store;
    }
    ...
    public function js($js_name, $type, $placeholder = 'tf_head', $priority = 10, $version = '1.0', $visibility = array())
    {
        ...
        if (!in_array($to_store, $this->js))
            if (file_exists($this->types[$type] . '/' . $js_name . '.js'))
                $this->js[] = $to_store;
    }
    ...
    public function include_all($buffer) {
        ...
        foreach ($placeholders_css as $name => $css_arr)
        {
            ...
            {
                if (!$this->pass_visibility_rules($css['visibility']))
                    continue;
        ...
        foreach ($placeholders_js as $name => $js_arr)
        {
            ...
            {
                if (!$this->pass_visibility_rules($js['visibility']))
                    continue;
    ...
    protected function pass_visibility_rules($rules)
    {
        ...
        if(gettype($current_screen) != 'object')
            return true; // maybe here should be an error or other checking, but I decided just to pass verification. If you have some ideas, change..
