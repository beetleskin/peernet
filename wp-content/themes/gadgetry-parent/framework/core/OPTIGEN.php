...
class TF_OPTIGEN extends TF_TFUSE {
    ...
    function selectsearch($opts)
    {
        ...
        if(!$this->include->type_is_registered('chosen_js')){
            $this->include->register_type('chosen_js', TFUSE . '/static/javascript');
            $this->include->js('chosen.jquery.min', 'chosen_js', 'tf_head', 10, '0.9.8');
            $this->include->js('chosen_tfuse', 'chosen_js', 'tf_head', 10, '1.0');
            $this->include->register_type('chosen_css', TFUSE . '/static/css');
            $this->include->css('chosen', 'chosen_css', 'tf_head', '0.9.8');
    ...
    function maps($opts)
    {
        ...
        $output .= '<script type="text/javascript">jQuery(document).ready(function($){ tf_init_google_maps_input("input.'.$uniqId.'"); });</script>';
...
//        );
    ...
    function table($opts=null){
        ...
        if(!empty($opts)){
            ...
            $this->include->js('tfuse_render_table', 'tfuse_render_table_js', 'tf_head', 10, '0.9.8');

            $str='';
            $str.= "<table class='tfuse-optigen-table ".$this->set_val($opts['class'])."' id='".$opts['id']."' style='".$this->set_val($opts['style'])."'>";
            $str.="<thead>";
            foreach($opts['columns'] as $th)
                $str.='<th class="'.$th['id'].'">'.$this->set_val($th['name']).(empty($th['desc'])? ' ' :' <a href="#" class="tfuse-tip-twitter" title="'.addslashes($this->set_val($th['desc'])).'" style="cursor:help">[?]</a>'). "</th>";
            $str.="</thead>";
            ...
            foreach($opts['value'] as $value){
                ...
                foreach($opts['columns'] as $key=>$td){
                    ...
                    if (method_exists($this->optigen, $td['type']))
                        $str.=$this->optigen->{$td['type']}(array_merge($td,array('value'=>$value[$key])));
                    else
                        die('This method doesn\'t exists '.$td['type'].'  in OPTIGEN Class . Error throw by method : '.__METHOD__);
                    $str.="</td>";
                }
                $str.="</tr>";
            }
            ...
            if(!empty($opts['default_value'])){
                ...
                foreach($opts['columns'] as $key=>$td){
                    $str.="<td style='overflow:visible' data-id='".$td['id']."' data-type='".$td['type']."'>";//
                    if (method_exists($this->optigen, $td['type']))
                        $str.=$this->optigen->{$td['type']}(array_merge($td,array('value'=>$opts['default_value'][$key])));
                    else
                        die('This method doesn\'t exists '.$td['type'].'  in OPTIGEN Class . Error throw by method : '.__METHOD__);
                    $str.="</td>";
                }
            ...
            $str.="<tfoot>";
            foreach($opts['columns'] as $th)
                $str.="<th >".$this->set_val($th['name'])."</th>";
            $str.="</tfoot>";
    ...
    function div_table($opts=null){
        ...
        $str= '';
        $str.="<div class='tfuse-optigen-div-table ".$this->set_val($opts['class'])."'   id='".$opts['id']."' style='".$this->set_val($opts['style'])."' >\n";
        ...
        foreach($opts['value'] as $value){
            ...
            foreach($opts['columns'] as $key=>$td){
                ...
                $str.=$this->interface->meta_box_row_template(array_merge($td,array('value'=>$value[$key])));
        ...
        if(!empty($opts['default_value'])){
            ...
            foreach($opts['columns'] as $key=>$td){
                ...
                $str.="<div class='div-table-td'  data-id='".$td['id']."' data-type='".$td['type']."'>";
                $str.=$this->interface->meta_box_row_template(array_merge($td,array('value'=>$opts['default_value'][$key])));
                $str.="<div style='clear:both'></div></div>\n";
            }
            $str.="<div style='clear:both'></div></div>\n";
        }

        $str.="<a class='add button tfbtq_shopping_add_row' href='#'>".(empty($opts['btn_labels'][0])?'Add Row':$opts['btn_labels'][0])."</a><a class='add button tfbtq_shopping_delete_rows' href='#'>".(empty($opts['btn_labels'][1])?'Delete Row':$opts['btn_labels'][1])."</a>";
        $str.="<div style='clear:both'></div></div>\n";
        $str.="<input type='hidden' name='".$opts['id']."'/>";
        $str.="<div style='clear:both'></div></div>\n";
        $str.="<script type='text/javascript'>";
