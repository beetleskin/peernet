...
class TF_FRONT_END_MENU_WALKER extends Walker_Nav_Menu {
    ...
    private function get_megafied_nav_ids() {
        ...
        foreach ($query_rez as $q) {
            ...
            if (isset($mega_opts[$this->mm_prefix . 'is_mega'])
                    && $mega_opts[$this->mm_prefix . 'is_mega'] == 'true')
                $return_arr[] = $q->post_id;
        }
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$megamenu_meta = get_post_meta($item->ID, $this->mm_meta_key, true);
		$is_megafied_parent_li = in_array($item->ID, $this->megafied_navs_ids);
		$is_megafied_child_li = $depth == $this->mm_magic_depth && $megamenu_meta != '';
	...
	private function get_all_children_of_nav($nav_id, $accumulator) {
	    if (!isset($this->children_holder[$nav_id])) {
	        //return array_merge($accumulator, array('children' => null));
	...
	function display_element($element, &$children_elements, $max_depth, $depth=0, $args, &$output) {
	    ...
	    if (in_array($element->ID, $this->megafied_navs_ids))
...
class TF_ADMIN_MENU_WALKER extends Walker_Nav_Menu {
 ...
 ?></textarea>
				...
				<?php do_action('tf_ext_megamenu_admin_nav_options', $depth, $item_id, $parent_id);?>
