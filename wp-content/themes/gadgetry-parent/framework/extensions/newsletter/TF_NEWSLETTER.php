...
class TF_NEWSLETTER extends TF_TFUSE {
    ...
    function add_menu() {
        add_submenu_page('themefuse', 'Newsletter Settings', 'Newsletter', 'manage_options', 'tf_newsletter', array(&$this, 'display'));
    }
    ...
    function mailchimp() {
        ...
        $this->include->js_enq('newsletter_service_name', 'MailChimp');
    }
    ...
    function campaignmonitor() {
        ...
        $this->include->js_enq('newsletter_service_name', 'CampaignMonitor');
    }
    ...
    function tfuse_ajax_newsletter_save_api_key() {
        ...
        if ($_POST['service'] != 'none') {
            $this->{'tfuse_ajax_newsletter_save_' . strtolower($_POST['service']) . '_api_key'}();
        }
        else
    ...
    function tfuse_ajax_newsletter_save_mailchimp_api_key() {
        ...
        $api->ping();
        if ($api->errorCode)
            tfjecho(array('status' => -1, 'message' => 'API key is invalid.'));
        else {
    ...
    function tfuse_ajax_newsletter_save_campaignmonitor_client() {
        ...
        tf_can_ajax();
        if ($this->cm_clients) {
            $client = $this->cm_clients->set_client_id($_POST['client_id'])->get();
            if ($client->http_status_code == 200) {
                $this->model->set_cm_client($_POST['client_id']);
                tfjecho(array('status' => 1));
            } else {
                tfjecho(array('status' => -1, 'message' => 'Invalid client ID or somethinf else went wrong.'));
            }
        }
        else
            tfjecho(array('status' => -1, 'message' => 'Could not connect to the CampaignMonitor API service.'));
        die();
    }
    ...
    function tfuse_ajax_newsletter_save_mailchimp_list() {
        ...
        tf_can_ajax();
        if ($this->mc_api) {
            $list = $this->mc_api->lists(array('list_id' => $_POST['list_id']));
            if (!$this->mc_api->errorCode) {
                $this->model->set_mc_list($_POST['list_id']);
                tfjecho(array('status' => 1));
            } else {
                tfjecho(array('status' => -1, 'message' => 'Invalid list ID or somethin else went wrong.'));
            }
        }
        else
            tfjecho(array('status' => -1, 'message' => 'Could not connect to the MailChimp API service.'));
        die();
    }
    ...
    function tfuse_ajax_newsletter_save_campaignmonitor_list() {
        ...
        tf_can_ajax();
        if ($this->cm_lists) {
            $list = $this->cm_lists->set_list_id($_POST['list_id'])->get();
            if ($list->http_status_code == 200) {
                $this->model->set_cm_list($_POST['list_id']);
                tfjecho(array('status' => 1));
            } else {
                tfjecho(array('status' => -1, 'message' => 'Invalid list ID or somethin else went wrong.'));
            }
    ...
    function tfuse_ajax_newsletter_fetch_emails_subscribed_mailchimp() {
        ...
        if ($this->mc_api) {
            ...
            if (!$list_id)
                tfjecho(array('status' => -1, 'message' => 'List ID is not set. Please select your list ID.'));
        ...
        if ($this->cm_lists) {
            ...
            do {
                ...
                if ($members->http_status_code != 200)
                    tfjecho(array('status' => -1, 'message' => 'Could not fetch subscribed members list. Check your API key.'));
    ...
    function tfuse_ajax_newsletter_save_email() {
        ...
        if (!$newsletter_service || $newsletter_service == 'none') {
            tfjecho(array('status' => -1, 'message' => 'Newsletter service not configured.'));
        }
        if (!is_email($_POST['email']))
            tfjecho(array('status' => -2, 'message' => 'Invalid email provided.'));
        ...
        if ($this->{'save_email_' . $newsletter_service}($_POST['email']))
            tfjecho(array('status' => 1));
        else
            tfjecho(array('status' => -1, 'message' => 'Could not save this email with the newsletter service.'));
        die();
    }
    ...
    function tfuse_ajax_newsletter_save_service() {
        tf_can_ajax();
        if ($_POST['service'] == '')
            return(array('status' => -1, 'message' => 'No newsletter service provided.'));
        if (!in_array($_POST['service'], $this->valid_services))
            return(array('status' => -1, 'message' => 'Invalid Service Provided.'));
        $this->model->set_service($_POST['service']);
        return 1;
    }
