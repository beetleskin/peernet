...
if (!function_exists('tfuse_postlist_view')) :	
	...
	$topbar =  isset($_GET['navibar']) ? $_GET['navibar'] : "";
        ...
        $str = substr($permalink,$pos,strlen($str));
            if(!isset($_COOKIE["themes_view"])) $_COOKIE["themes_view"] = 'grid';
...
if (!function_exists('tfuse_show_list_before')) :	
    function tfuse_show_list_before() {
        $view =  isset($_GET['view']) ? $_GET['view'] : "";
        $topbar =  isset($_GET['navibar']) ? $_GET['navibar'] : "";
        if(($_COOKIE["themes_view"]=='grid' && $view!='list' ) || $view == 'grid' )
        {
...
if (!function_exists('tfuse_type_logo')) :
    function tfuse_type_logo() { 
        ...
        {  ?> 
               <a href="<?php bloginfo('url'); ?>"><strong><?php echo tfuse_options('logo_text'); ?></strong></a>
                <div class="logo-sub"><?php echo tfuse_options('logo_text_subtitle'); ?></div>
       <?php }
        elseif(tfuse_options('choose_logo') == 'custom')
        { ?>
            <a href="<?php bloginfo('url'); ?>"><div class="logo-img"><img src="<?php echo tfuse_logo(); ?>" alt="<?php bloginfo('name'); ?>"  border="0" /></div></a>
       <?php }
	   else 
	   { ?>
			<a href="<?php bloginfo('url'); ?>"><strong><?php echo tfuse_options('logo_text','Gadgetry'); ?></strong></a>
...
if (!function_exists('tfuse_site_style')) :
    function tfuse_site_style() {
        if (isset($_GET['color']))$color_scheme = $_GET['color'];
			...
			if ($attachments) {
			 foreach ($attachments as $attachment) { 
                        ...
                        else
                        {
                            ...
                            $pos2 = strpos($content,$str2);
                            if(is_numeric($pos1))
                            { 
                                $str = substr($content,$pos1,$pos2);
                                if(!empty($str))
                                {   
